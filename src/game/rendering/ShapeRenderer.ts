...
export class ShapeRenderer {
  ...
  public static renderShapePreview(
    type: string,
    ...
    switch (type) {
      case 'rectangle':
        ctx.fillRect(position.x - size/2, position.y - size/3, size, size/1.5);
        ctx.strokeRect(position.x - size/2, position.y - size/3, size, size/1.5);
        break;
      ...
      case 'star':
        this.drawPolygon(ctx, position.x, position.y, 5, size/2);
        ctx.fill();
        ctx.stroke();
        break;
    }
