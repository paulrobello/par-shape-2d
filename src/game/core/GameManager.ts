...
=== SCREW ${inputType.toUpperCase()} INPUT DEBUG ===`);
  ...
  private renderGameOverCountdown(): void {
    ...
    this.ctx.fillText('Clear some space or game will end...', this.canvas.width / 2, this.canvas.height / 2 + 40);
  }
  ...
  private renderGame(): void {
    ...
    this.renderHUD();
    this.renderContainerHoldingBackground();
  ...
  private renderHUD(): void {
    ...
    // Calculate screen-aware scaling for header elements
    const canvasWidth = this.canvas?.width || GAME_CONFIG.canvas.width;
    const originalWidth = GAME_CONFIG.canvas.width;
    
    // Scale header elements based on actual canvas width relative to original design
    const headerScale = Math.min(1.2, canvasWidth / originalWidth); // Allow slight upscaling but cap it
    ...
    const baseFontSize = 18; // Slightly smaller base for better mobile fit
    const fontSize = Math.max(14, Math.min(24, baseFontSize * headerScale)); // Bounded font size
    const margin = Math.max(15, 20 * headerScale);
    const lineHeight1 = 30;
    const lineHeight2 = 55;
    ...
    if (this.gameOverTimer !== null) {
      ...
      this.ctx.textAlign = 'center';
      this.ctx.fillStyle = '#FF4444';
      this.ctx.font = `bold ${Math.min(fontSize + 2, 20)}px Arial`;
      this.ctx.fillText(`HOLES FULL! ${countdown}s`, this.virtualGameWidth / 2, lineHeight1);
      
      this.ctx.fillStyle = '#FFAA44';
      this.ctx.font = `${Math.max(10, fontSize * 0.75)}px Arial`;
      this.ctx.fillText('Clear space!', this.virtualGameWidth / 2, lineHeight1 + 18);
    }
  ...
  private renderContainerHoldingBackground(): void {
    if (!this.ctx || !this.canvas) return;
  ...
  private renderContainers(): void {
    ...
    containers.forEach(container => {
      ...
      this.ctx!.beginPath();
      this.ctx!.roundRect(position.x - 60, position.y - 24, 120, 48, 8);
      this.ctx!.fill();
      ...
      if (container.isMarkedForRemoval && container.removalTimer !== undefined) {
        ...
        this.ctx!.beginPath();
        this.ctx!.roundRect(position.x - 62, position.y - 26, 124, 52, 10);
        this.ctx!.stroke();
      }
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (container.holes[i]) {
          ...
          this.ctx!.beginPath();
          this.ctx!.arc(holeX, holeY, UI_CONSTANTS.screws.radius - 2, 0, Math.PI * 2);
          this.ctx!.fill();
  ...
  private renderHoldingHoles(): void {
    ...
    holes.forEach(hole => {
      ...
      this.ctx!.beginPath();
      this.ctx!.arc(position.x, position.y, UI_CONSTANTS.screws.radius - 2, 0, Math.PI * 2);
      this.ctx!.fill();
  ...
  private renderGameArea(): void {
    ...
    this.ctx.fillRect(0, 158, this.virtualGameWidth, this.virtualGameHeight - 158);
  ...
  public start(): void {
    ...
    if (hasGameInProgress) {
      ...
      console.log('Full save data exists:', !!fullSaveData);
      
      if (fullSaveData) {
        console.log('Loading full game state with layers and screws');
        console.log('Full save data structure:', {
          hasGameState: !!fullSaveData.gameState,
          hasLayerManagerState: !!fullSaveData.layerManagerState,
          layerCount: fullSaveData.layerManagerState?.layers?.length || 0,
          hasScrewManagerState: !!fullSaveData.screwManagerState
        });
        
        // Restore layer manager state
        this.layerManager.fromSerializable(fullSaveData.layerManagerState);
        
        // Restore screw manager state
        this.layerManager.getScrewManager().fromSerializable(fullSaveData.screwManagerState);
        
        // Update layer bounds for current canvas scaling after restore
        this.updateLayerBoundsForScale();
        
        console.log(`Restored ${fullSaveData.layerManagerState.layers.length} layers with full state`);
        console.log('Shapes after restore:', this.layerManager.getAllShapes().length);
      } else {
        console.log('No full save data found, initializing level from basic state');
        const state = this.gameState.getState();
        
        // Fallback: initialize level if no shapes exist
        if (this.layerManager.getAllShapes().length === 0) {
          console.log('No shapes found, initializing new level');
          
          // Update layer bounds BEFORE creating shapes to ensure they use the full area
          this.updateLayerBoundsForScale();
          
          this.layerManager.initializeLevel(state.currentLevel);
          ...
          this.gameState.updateContainersFromActiveScrewColors(activeScrewColors);
        }
      }
  ...
  private updateCanvasScaling(): void {
    ...
    // Update layer bounds to account for the virtual dimensions
    this.updateLayerBoundsForScale();
  }
  ...
  private getCurrentPlayableBounds(): { x: number; y: number; width: number; height: number } {
    ...
    const bounds = {
      ...
      height: this.virtualGameHeight - headerHeight - minMargin
    };
    
    console.log(`GET_CURRENT_PLAYABLE_BOUNDS: virtual=${this.virtualGameWidth.toFixed(0)}x${this.virtualGameHeight.toFixed(0)}, playable=(${bounds.x}, ${bounds.y}, ${bounds.width.toFixed(0)}, ${bounds.height.toFixed(0)}) Y_RANGE=${bounds.y} to ${bounds.y + bounds.height}`);
    
    return bounds;
  }

  private updateLayerBoundsForScale(): void {
    if (!this.canvas) return;
    ...
    console.log(`BOUNDS CALCULATION: headerHeight=158, minMargin=10, playableY=${playableBounds.y}, playableHeight=${playableBounds.height}, playableYEnd=${playableBounds.y + playableBounds.height}`);
    ...
    this.layerManager.getLayers().forEach(layer => {
      ...
      layer.updateBounds(playableBounds);
    });
  }

  private getVisibleArea(): { left: number; top: number; right: number; bottom: number; width: number; height: number } {
    // With the new scaling approach using Math.min(), the entire original canvas
    // is always visible, so we can simply return the original dimensions
    const originalWidth = GAME_CONFIG.canvas.width;
    const originalHeight = GAME_CONFIG.canvas.height;
